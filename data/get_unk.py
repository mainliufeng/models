...
def main(_):
  tokenizer = FullTokenizer(FLAGS.vocab_file, do_lower_case=FLAGS.do_lower_case)
  processor = StsProcessor()
  with open("token_check.txt", "w") as output_file:
    for example in processor.get_examples(
      FLAGS.input_data_dir, FLAGS.set_type):
      tokens = single_example_to_tokens(
        example, FLAGS.max_seq_length, tokenizer)
      if "[UNK]" in tokens:
        output_file.write("{}\n".format(" ".join(tokens)))


def single_example_to_tokens(example, max_seq_length, tokenizer):
